...
class HttpRequest {
  ...
  // success：请求成功回调
  // error：请求失败回调
  Future request<T>(String path, {Map<String, dynamic> params, Method method = Method.get}) async {
    try {
  ...
  // success：请求成功回调
  // error：请求失败回调
  Future requestList<T>(String path, {Map<String, dynamic> params, Method method}) async {
    try {
      Response response = await dio.request(path, queryParameters: params, options: Options(method: method.value));
      if (response != null) {
